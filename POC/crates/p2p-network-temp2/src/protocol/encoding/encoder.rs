...
macro_rules! wire_message {
    ($enum_name:ident {
        ...
        impl $enum_name{
            fn to_wire(&self) -> Result<(u8, Vec<u8>), crate::P2PError>{
                match self {
                    $(
            ...
            fn from_wire(type_id: u8, payload: &[u8]) -> Result<Self, crate::P2PError>
            where 
                $($type: crate::protocol::encoding::WireEncodable, )*
            {
                match type_id{
                    $(
                        $id => Ok($enum_name::$variant(<$type>::decode(payload)?)),
                    )*
                    _ => Err(crate::P2PError::UnknownMessageType(type_id))
                }
            }
        }
    }
